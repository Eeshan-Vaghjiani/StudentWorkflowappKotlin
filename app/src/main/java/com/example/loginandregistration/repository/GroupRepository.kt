...
class GroupRepository {
        ...
        suspend fun getPublicGroups(): Result<List<FirebaseGroup>> =
                withContext(Dispatchers.IO) {
                        val userId =
                                auth.currentUser?.uid
                                        ?: return@withContext Result.success(emptyList())
                        return@withContext safeFirestoreCall("getPublicGroups") {
                                groupsCollection
        ...
        suspend fun joinGroupByCode(code: String): Boolean =
                withContext(Dispatchers.IO) {
                        ...
                        return@withContext try {
                                ...
                                // Create updated group object for validation
                                val updatedMembers =
                                        group.members.toMutableList().apply { add(newMember) }
                                val updatedMemberIds =
                                        group.memberIds.toMutableList().apply { add(user.uid) }
                                val updatedGroup =
                                        group.copy(
                                ...
                                val validationResult = validator.validateGroup(updatedGroup)
                                if (!validationResult.isValid) {
                                        Log.e(
                                                "GroupRepository",
                                                "Validation failed when joining group by code: ${validationResult.getErrorMessage()}"
                                        )
                                        return@withContext false
                                }
                                ...
                                addGroupActivity(
                                        ...
                                        description =
                                                "${user.displayName ?: "A new member"} joined the group"
                                )
                ...
                withContext(Dispatchers.IO) {
                        ...
                        return@withContext try {
                                ...
                                if (!validationResult.isValid) {
                                        Log.e(
                                                "GroupRepository",
                                                "Validation failed when joining public group: ${validationResult.getErrorMessage()}"
                                        )
                                        return@withContext false
                                }
                ...
                withContext(Dispatchers.IO) {
                        ...
                        return@withContext try {
                                ...
                                addGroupActivity(
                                        ...
                                        title = "Member left",
                                        description =
                                                "${user.displayName ?: "A member"} left the group"
                                )
        ...
        fun getUserGroupsFlow(): Flow<List<FirebaseGroup>> =
                callbackFlow {
                                ...
                                if (userId == null) {
                                        Log.d(
                                                "GroupRepository",
                                                "No authenticated user, returning empty list"
                                        )
                                        trySend(emptyList())
                                        close()
                                        return@callbackFlow
                                        ...
                                        groupsCollection
                                                ...
                                                .addSnapshotListener { snapshot, error ->
                                                        if (error != null) {
                                                                ...
                                                                // Emit error state with appropriate
                                                                // message
                                                                val errorMessage =
                                                                        when {
                                                                                error.message
                                ...
                                .addSnapshotListener { groupSnapshot, error ->
                                        if (error != null) {
                                                ...
                                                if (error.message?.contains(
                                                        ...
                                                        // Send empty stats instead of closing the
                                                        // flow
                                                        // This prevents crashes and allows the UI
                                                        // to show an empty
                                                        // state
                                                        trySend(GroupStats())
                                                } else {
                                                        // For other errors, send empty stats and
                                                        // continue listening
                                                        trySend(GroupStats())
                                                }
                                                return@addSnapshotListener
                                        ...
                                        Log.d(
                                                "GroupRepository",
                                                "Group count update: $myGroupsCount groups"
                                        )
                                        trySend(GroupStats(myGroups = myGroupsCount))

                                        // If there are groups, fetch additional stats
                                        // asynchronously
                ...
                try {
                        ...
                        activitiesCollection
                                ...
                                .addOnSuccessListener { activitySnapshot ->
                                        val activities =
                                                activitySnapshot.toObjects(
                                                        GroupActivity::class.java
                                                )
                                        val newMessages =
                                                activities.filter { it.type == "message" }.size

                                        // TODO: Calculate active assignments when task-group
        ...
        suspend fun getGroupById(groupId: String): FirebaseGroup? =
                withContext(Dispatchers.IO) {
                        return@withContext try {
                                // Check cache first
                                com.example.loginandregistration.utils.DataCache.getGroup(groupId)
                                        ?.let {
                                                Log.d(
                                                        "GroupRepository",
                                                        "Returning cached group: $groupId"
                                                )
                                                return@withContext it
                                        }

                                Log.d("GroupRepository", "Fetching group from Firestore: $groupId")
                                val document = groupsCollection.document(groupId).get().await()
                                val group = document.toObject(FirebaseGroup::class.java)

                                // Cache the group
                                group?.let {
                                        com.example.loginandregistration.utils.DataCache.cacheGroup(
                                                groupId,
                                                it
                                        )
                                }
        ...
        // Helper method to invalidate group cache when group is modified
        private fun invalidateGroupCache(groupId: String) {
                com.example.loginandregistration.utils.DataCache.invalidateGroup(groupId)
                // Also invalidate group membership cache for current user
                auth.currentUser?.uid?.let { userId ->
                        com.example.loginandregistration.utils.DataCache.invalidateGroupMembership(
                                userId
                        )
                }
        }
                                ...
                                val listener =
                                        activitiesCollection
                                                ...
                                                .addSnapshotListener { snapshot, error ->
                                                        ...
                                                        trySend(
                                                                RepositoryResult.Success(activities)
                                                        )
                                                }
        ...
        suspend fun removeMemberFromGroup(groupId: String, memberId: String): Boolean {
                ...
                try {
                        ...
                        if (user != null) {
                                val memberName =
                                        group.members.find { it.userId == memberId }?.displayName
                                                ?: "A member"
                                addGroupActivity(
                                        groupId = groupId,
                ...
                try {
                        ...
                        val updatedGroup =
                                group.copy(
                                        ...
                                        subject = subject,
                                        settings =
                                                group.settings.copy(isPublic = privacy == "public"),
                                        updatedAt = Timestamp.now()
                                )
                        ...
                        if (user != null) {
                                addGroupActivity(
                                        ...
                                        description =
                                                "${user.displayName ?: "Someone"} updated the group details",
                                        type = "group_updated"
                                )
                        }
        ...
        suspend fun addMemberToGroup(groupId: String, userId: String): Boolean {
                ...
                try {
                        ...
                        val targetUser =
                                userSnapshot.toObject(FirebaseUser::class.java) ?: return false
                        ...
                        if (!validationResult.isValid) {
                                Log.e(
                                        "GroupRepository",
                                        "Validation failed when adding member: ${validationResult.getErrorMessage()}"
                                )
                                return false
                        }
